...
function validatePassword(password: string): boolean {
  const rules: Array<(pwd: string) => boolean> = [
    (pwd) => password.length >= 8,         
    (pwd) => password.trim().length > 0,
    (pwd) => /[A-Z]/.test(password),       
    (pwd) => /[a-z]/.test(password),       
    (pwd) => /\d/.test(password),          
  ];
...
console.log(validatePassword("PWO"));  
console.log(validatePassword("Pawd 333"));
